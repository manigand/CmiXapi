...
class ilCmiXapiLaunchGUI
{
    ...
    protected function launchCmd()
    {
        ...
        $this->log()->debug('début launchCmd');
        $this->initCmixUser();
        $token = $this->getValidToken();
        if ($this->object->getContentType() == ilObjCmiXapi::CONT_TYPE_CMI5) {
        $this->log()->debug('launchCmd dans if');
            $ret = $this->CMI5preLaunch($token);
            $token = $ret['token'];
            $this->log()->debug('ret : '.var_export($ret,true));
        }
        $launchLink = $this->buildLaunchLink($token);
        $this->log()->debug('fin launchCmd : '.$launchLink);
  ...
  //      $params['activity_id'] = urlencode($this->object->getActivityId());   //modif
    ...
    {
        global $DIC;
        $this->log()->debug('debut CMIprelaunch');
        ...
        $this->log()->debug('CMIpreLaunch step1');
        $registration = $this->cmixUser->getRegistration();
        // for old CMI5 Content after switch commit but before cmi5 bugfix
        if ($registration == '') {
            $registration = ilCmiXapiUser::generateRegistration($this->object, $DIC->user());
        }
        $this->log()->debug('CMIpreLaunch step 2');
        ...
        $this->log()->debug('CMIpreLaunch step3');
        // launchData
        $launchDataParams = [];
        ...
        $abandoned = false;
        $this->log()->debug('CMIpreLaunch step4');
        ...
        $this->log()->debug('CMIpreLaunch step5');
        // satisfied on launch?
...
$this->log()->debug('CMIpreLaunch step6');
        ...
        $this->log()->debug('CMIpreLaunch step7');
        // launchedStatement
        ...
        $this->log()->debug('CMIpreLaunch step8');
        // abandonedStatement
        if ($abandoned) {
        ...
        $this->log()->debug('CMIpreLaunch step9');
        // abandonedStatement
        if ($satisfied) {
        ...
        $this->log()->debug('CMIpreLaunch step10');
        ...
        $this->log()->debug('CMIpreLaunch step11');
        $promises = array();
        ...
        $this->log()->debug('CMIpreLaunch step12 avant return');
